...
__global__ void rk4(GPU::numtype* tinit,GPU::complexnumtype* yi,GPU::complexnumtype* yf,int node,int ntime,GPU::numtype h,OdeRhs* F)
{
    int idx = blockIdx.x*blockDim.x + threadIdx.x;
    
    if(idx<node){
        printf("Hello! (idx,tinit[idx],yi[idx]) = (%i,%f,%f)\n",idx,tinit[idx],yi[idx]);
        yf[idx] = rk4kernel(tinit[idx],yi[idx],ntime,h,F[idx]);
        printf("Result[idx] = %f\n",yf[idx]);
    }
}
