...
// any purpose.
// ---------------------------------------------------------------------------
#ifndef __CGRI_OBJECTS_LUMPY_AUC_KERNEL_GPU_H__
#define __CGRI_OBJECTS_LUMPY_AUC_KERNEL_GPU_H__
#include <math.h>
//#include "util.h"
//#define POW2 2.0

template<typename T>
__device__ T AUCLump(int dim, T* R, T* R0, T tau, T D)
{
    // Evaluates an "diffusion AUC" lump auc(r,t;r0,t0)
    ...
    if((D>0)&&(exponent>0)&&(tau>0)){
        return erfc(sqrt(exponent/(4.0*D*tau)))/(4.0*M_PI*D*sqrt(exponent)); 
    } else{
        return 0;   // I think this is OK since the function is integrable on R^d d>2
    }
}

template<typename T>
__global__ void LumpyAUCKernel(int dim,int K,int nEval,T* centers,T* evalpts,T* timepts,T evaltime,T* result,T C0,T D)
{
	...
	if(tId<nEval){
        ...
        {
            result[tId] += AUCLump(dim,&evalpts[idx],&centers[dim*iLump],evaltime-timepts[iLump],D);
        }
	}
}
