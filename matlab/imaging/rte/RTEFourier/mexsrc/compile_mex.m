...
% File: matlab/imaging/rte/mexsrc/compile_mex.m
% Purpose: Matlab script to compile the rte imaging simulator
% Notes: 
...
switch GPU
    case 1
        ...
        str = [GPUFLAGS,' rte_fourier_mex_gpu.cu'];
        args = regexp(str,'\s+','split');
        mexcuda(args{:})    % for R2016 and above
        movefile(strcat('rte_fourier_mex_gpu',mexext),strcat('../mexbin/rte_fourier_mex_gpu',mexext));
        str = [CPUFLAGS,' rte_fourier_mex_cpu.cc'];
        args = regexp(str,'\s+','split');
        mex(args{:})
        movefile(strcat('rte_fourier_mex_cpu',mexext),strcat('../mexbin/rte_fourier_mex_cpu',mexext));
        cd ../
