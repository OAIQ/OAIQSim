...
switch GPU
    case 1
        ...
        str = [GPUFLAGS,' rte_lumpy_mex_gpu.cu'];
        args = regexp(str,'\s+','split');
        mexcuda(args{:})    % for R2016 and above
        movefile(strcat('rte_lumpy_mex_gpu',mexext),strcat('../mexbin/rte_lumpy_mex_gpu',mexext));
        str = [CPUFLAGS,' rte_lumpy_mex_cpu.cc'];
        args = regexp(str,'\s+','split');
        mex(args{:})
        movefile(strcat('rte_lumpy_mex_cpu',mexext),strcat('../mexbin/rte_lumpy_mex_cpu',mexext));
        cd ../
