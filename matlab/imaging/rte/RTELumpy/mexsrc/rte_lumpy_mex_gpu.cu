...
{   
    // Need to update the LumpyBgnd object so i'm not accessing these directly (need to validate inputs!)
    Lbg->dim = dim;
    Lbg->K = (int)mxGetScalar(mxGetField(input,0, "K"));
    Lbg->B0 = (double)mxGetScalar(mxGetField(input,0, "B0"));
    Lbg->b0 = (double*)mxGetData(mxGetField(input,0, "b0"));
    Lbg->rb2 = (double*)mxGetData(mxGetField(input,0, "rb2"));
    Lbg->centers = (double*)mxGetData(mxGetField(input,0, "centers"));
}
...
{   
    ...
    mexPrintf("Successfully gathered all inputs!\n");
