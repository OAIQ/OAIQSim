...
% File: /matlab/objects/diffusion/mexsrc/compile_mex.m
% Purpose: Matlab script to compile the lumpy diffusion mex files
% Notes: 
...
switch GPU
    case 1
        ...
        str = [GPUFLAGS,' lumpy_diff_mex_gpu.cu'];
        args = regexp(str,'\s+','split');
        mexcuda(args{:})    % for R2016 and above
        movefile(strcat('lumpy_diff_mex_gpu',mexext),strcat('../mexbin/lumpy_diff_mex_gpu',mexext));
        str = [CPUFLAGS,' lumpy_diff_mex_cpu.cc'];
        args = regexp(str,'\s+','split');
        mex(args{:})
        movefile(strcat('lumpy_diff_mex_cpu',mexext),strcat('../mexbin/lumpy_diff_mex_cpu',mexext));
        cd ../
